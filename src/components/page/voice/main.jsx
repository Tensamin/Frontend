...
export function Main({ data }) {
    ...
    useEffect(() => {
        ...
        handleMessage();
    }, [lastMessage, localUserId, createNewPeerConnection, send]);

    // Identification
    useEffect(() => {
        if (isMediaReady && readyState === ReadyState.OPEN && !identified) {
            log(
            ...
            async function asyncSend() {
                send({
                    ...
                    data: {
                        ...
                        call_secret_sha: await sha256(jsonData.secret),
                    },
                });
            }
            asyncSend()
        }
    }, [isMediaReady, readyState, identified, privateKeyHash, send, localUserId]);
