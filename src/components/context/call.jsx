...
export let CallProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      sinkObserverRef.current.observe(document.body, {
        childList: true,
        subtree: true,
      });
      return () => {
  ...
  // mic switch
  let switchMicInput = useCallback(
    async (preferredId) => {
      try {
        let base = {
      ...
      switch (message.type) {
        ...
        case "client_closed":
          ...
          setConnectedUsers((prev) =>
            prev.filter((userId) => userId !== message.data.user_id),
          );
        ...
        case "webrtc_sdp":
          ...
          try {
            sdp_payload = atob(
              await decrypt_base64_using_aes(message.data.payload, callSecret),
            );
          } catch (err) {
            log(`${sdp_sender}: ${err.message}`, "error", "Voice WebSocket:");
            return;
          }
          ...
          if (!sdp_pc) {
            sdp_pc = await createP2PConnection(
              sdp_sender,
              false,
              sdp_isScreenShare,
            );
          ...
          try {
            ...
            if (sdp_obj.type === "offer") {
              ...
              log(`Answer sent to ${sdp_sender}`, "success");
            }
          } catch (error) {
            log(
    ...
    createCall && callId && callSecret ? endpoint.call_wss : null,
    {
      ...
      onClose: () => {
        ...
        pendingRequests.current.forEach(({ reject, timeoutId }) => {
          ...
          reject(
            new Error(
              "Call WebSocket connection closed before response was received.",
            ),
          );
        });
          ...
          return new Promise((resolve, reject) => {
            ...
            let timeoutId = setTimeout(() => {
              ...
              let timeoutError = new Error(
                `Call WebSocket request timed out for ID: ${id} (Type: ${requestType}) after ${responseTimeout}ms.`,
              );
              reject(timeoutError);
            }, responseTimeout);
            ...
            try {
              ...
              let sendError = new Error(
                `Failed to send Call WebSocket message: ${e.message}`,
              );
  ...
  let startScreenStream = useCallback(async () => {
    ...
    try {
      ...
      async function addScreenShareToPeers() {
        let addPromises = Array.from(watchingRefs.current.entries()).map(
          async ([id, pc]) => {
            try {
              if (
              ...
              let videoTransceiver =
                ...
                transceivers.find(
                  (t) => t.mid !== null && t.direction.includes("recv"),
                );
              ...
              logFunction(
                `Failed to add screen tracks to peer ${id}: ${err.message}`,
                "error",
                "Voice WebSocket:",
              );
            }
  ...
  let stopScreenStream = useCallback(() => {
    ...
    let removePromises = Array.from(watchingRefs.current.entries()).map(
      async ([id, pc]) => {
        try {
          ...
          tracksToRemove.forEach((trackInfo) => {
            if (trackInfo.kind === "video") {
              let videoTransceiver = transceivers.find(
                (t) => t.sender.track && t.sender.track.id === trackInfo.id,
              );
              ...
              if (videoTransceiver) {
                try {
                  ...
                  logFunction(
                    `Error resetting video transceiver for peer ${id}: ${err.message}`,
                    "error",
                    "Voice WebSocket:",
                  );
                }
              ...
              let sender = senders.find(
                (s) => s.track && s.track.id === trackInfo.id,
              );
              if (sender) {
                try {
                  ...
                  logFunction(
                    `Error removing ${trackInfo.kind} track from peer ${id}: ${err.message}`,
                    "error",
                    "Voice WebSocket:",
                  );
                }
          ...
          logFunction(
            `Error stopping screen stream for peer ${id}: ${err.message}`,
            "error",
            "Voice WebSocket:",
          );
        }
  ...
  // p2p connections
  let createP2PConnection = useCallback(
    async (id, isInitiator = false, isScreenShare = false) => {
      if (isScreenShare) {
        if (isInitiator) {
          if (watchingRefs.current.has(id)) {
            logFunction(
              "Reusing existing watching connection",
              "debug",
              "Voice WebSocket:",
            );
          ...
          if (screenRefs.current.has(id)) {
            logFunction(
              "Reusing existing screen connection",
              "debug",
              "Voice WebSocket:",
            );
        ...
        if (micRefs.current.has(id)) {
          logFunction(
            "Reusing existing mic connection",
            "debug",
            "Voice WebSocket:",
          );
      ...
      pc.ontrack = (event) => {
        ...
        if (isScreenShare && !isInitiator) {
          ...
          if (audioElement && stream instanceof MediaStream) {
            ...
            logFunction(
              `Audio element not yet available for user ${id}, will be set when element is created`,
              "debug",
            );
          }
        }
        ...
        send(
          ...
          if (data.type === "identification_response") {
            ...
            if (inviteOnNewCall) {
              let publicKey;
              if (receiver !== "") {
                publicKey = await get(receiver).then((data) => {
